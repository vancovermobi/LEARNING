...
export class MaterialComponent implements AfterViewInit, OnInit {
  ...
  ngOnInit(): void {
    this.employeeService.GetListUrl().subscribe((response: any) => {
      ...
      this.dataSource = new MatTableDataSource<EmployeeElement>(response);
    },
      (error) => {console.log('System error API ' + error);}
      );
  }
  displayedColumns: string[] = ['id', 'name', 'age', 'status', 'createdAt'];
  
  dataSourceOld = new MatTableDataSource<PeriodicElement>(ELEMENT_DATA);
  ...
  ngAfterViewInit() {
    this.dataSourceOld.paginator = this.paginator;
